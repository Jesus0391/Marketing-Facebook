...
namespace Facebook.Client
{
    public class FacebookClient : IFacebookClient
    {
        ...
        private string GetQueryString(object model)
        {
            ...
            else if (model.GetType().GetRuntimeProperties().Any(c=>c.GetCustomAttributes().Count() > 0))
            {
            ...
            {
                var queryString = "&" +
                    string.Join("&", model.GetType().GetRuntimeProperties().Select(p => GetFieldName(p) + "=" + GetFieldValue(p, model)));
